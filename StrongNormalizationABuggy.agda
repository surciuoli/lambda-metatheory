    ...
    where
        ...
        ... | no y≢x =
          let σ⇂P : σ ∶ Γ ⇀ Δ ⇂ P
              ...
              Pσ⇓ : ∃ λ z → SNe z (P ∙ σ)
              Pσ⇓ = proj₂ (proj₁ (SN-lemmaNe P⇓ (acc hi) P:γ→ε N⇓) σ⇂P Unyσ x:B) y≢x
              σ⇂Q : σ ∶ Γ ⇀ Δ ⇂ Q
              σ⇂Q = λ y*Q → σ⇂PQ (*·r y*Q)
              Qσ⇓ : SN (Q ∙ σ)
              Qσ⇓ = proj₁ (SN-lemma Q⇓ (acc hi) Q:γ N⇓) σ⇂Q Unyσ x:B
        ...
        thesis₁ {.y} {σ} {Δ} σ⇂PQ Unyσ y:B | yes refl =
          let σ⇂P : σ ∶ Γ ⇀ Δ ⇂ P
              σ⇂P = λ y*P → σ⇂PQ (*·l y*P)
              Pσ⇓ : SN (P ∙ σ)
              Pσ⇓ = proj₁ (proj₁ (SN-lemmaNe P⇓ (acc hi) P:γ→ε N⇓) σ⇂P Unyσ y:B)
              σ⇂Q : σ ∶ Γ ⇀ Δ ⇂ Q
              σ⇂Q = λ y*Q → σ⇂PQ (*·r y*Q)
              Qσ⇓ : SN (Q ∙ σ)
              Qσ⇓ = proj₁ (SN-lemma Q⇓ (acc hi) Q:γ N⇓) σ⇂Q Unyσ y:B
              Pσ:γ→ε : Δ ⊢ P ∙ σ ∶ γ ⟶ ε
          ...
          in proj₂ (SN-lemma Pσ⇓ (hi γ γ<β) Pσ:γ→ε Qσ⇓) (ε , refl) Qσ:γ , λ y≢x → ⊥-elim (y≢x refl)
                                  
SN-lemma {β = B} (sne M⇓) accB M:α N⇓ =
  let th₁ , th₂ = SN-lemmaNe M⇓ accB M:α N⇓
  in (λ p1 p2 p3 → proj₁ (th₁ p1 p2 p3)) , th₂ 
SN-lemma {ƛ y P} {Γ} {δ ⟶ ε} {B} {N} (abs P⇓) accB (⊢ƛ P:ε) N⇓ = thesis₁ , thesis₂ P:ε
  where thesis₁ : ∀ {x σ Δ} → σ ∶ Γ ⇀ Δ ⇂ ƛ y P → Unary σ x N → Γ ⊢ v x ∶ B → SN (ƛ y P ∙ σ)
        ...
        ... | no y≢x =
          let z : V
              ...
              Pσ,y=z⇓ : SN (P ∙ (σ ≺+ (y , v z)))
              Pσ,y=z⇓ = proj₁ (SN-lemma P⇓ accB P:ε N⇓) σ,y=z⇂P Unyσ,y=z x:B'
          in abs Pσ,y=z⇓
        thesis₁ {.y} {σ} {Δ} σ⇂ƛyP Unyσ y:B | yes refl =
          let z : V
              ...
              Pσ,y=z,u=w⇓ = proj₁ (SN-lemma P⇓ accB P:ε' (sne v)) σ⇂P Unyσ,y=z,u=w u:B
        ...
        thesis₂ {.B} {.γ} P:γ (γ , refl) N:B =
          let y=N⇂P : (ι ≺+ (y , N)) ∶ (Γ ‚ y ∶ B) ⇀ Γ ⇂ P
              ...
              Py=N⇓ = proj₁ (SN-lemma P⇓ accB P:γ N⇓) y=N⇂P Unyy=N Γ,y:B⊢y:B
          in exp (β N⇓) Py=N⇓
SN-lemma {M} {Γ} {α} {B} {P} (exp {.M} {N} M→N N⇓) accB M:α P⇓ = thesis₁ , thesis₂
  where M→αN : M →α* N
        M→αN = →SN⊂→α M→N
        N:α : Γ ⊢ N ∶ α
...
SN-theo (⊢· {α} {B} {M} M:α→β N:α) =
  let M⇓ = SN-theo M:α→β
  in proj₂ (SN-lemma M⇓ (wfₜ<⁺ α) M:α→β (SN-theo N:α)) (B , refl) N:α
